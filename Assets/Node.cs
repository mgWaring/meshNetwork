...
public class Node : MonoBehaviour
{
    ...
    private void RelayMessage(List<Node> peers)
    {
        foreach (Node peer in peers)
        {
            foreach (Message message in messages)
            {
                if (peer.NeedMessage(message.uID))
                {
                    ...
                    anim?.SetDestination(peer.transform.position);
    ...
    private void RelayDeadMessage(List<Node> peers)
    {
        foreach (Node peer in peers)
        {
            foreach (KeyValuePair<Guid, float> entry in deadMessages)
